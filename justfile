...
_repo-check:
    #!/usr/bin/env bash
    set -euo pipefail
    # Python backend: byte-compile all modules (syntax signal).
    if command -v python3 >/dev/null 2>&1 && compgen -G "backend/*.py" >/dev/null; then
      python3 -m py_compile backend/*.py
      echo "py_compile: backend OK"
    fi
    # Vite frontend: production build blocking; eslint advisory.
    if [[ -f frontend/package.json ]] && command -v npm >/dev/null 2>&1; then
      npm --prefix frontend run lint --if-present || echo "just check: eslint reported pre-existing issues (advisory)"
